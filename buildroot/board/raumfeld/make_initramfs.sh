...
INITRAMFS_DIR="${BUILDROOT_DIR}/board/raumfeld/initramfs_tree"
...
# Copy Libraries
cp -L "${TARGET_DIR}/lib/ld-linux-armhf.so.3" "${INITRAMFS_DIR}/lib/"
cp -L "${TARGET_DIR}/lib/libc.so.6" "${INITRAMFS_DIR}/lib/"
cp -L "${TARGET_DIR}/lib/libresolv.so.2" "${INITRAMFS_DIR}/lib/"
...
echo "Initramfs tree created at ${INITRAMFS_DIR}"
